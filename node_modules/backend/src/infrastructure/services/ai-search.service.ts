...
3. Đổi các cụm từ chỉ giá cả như "dưới 2 triệu" -> priceMax = 2000000, "khoảng 1.5 triệu" -> priceMin = 1300000, priceMax = 1700000.`;
  ...
  public async parseQuery(queryText: string): Promise<AiFilterResult> {
  ...
  private async callGemini(queryText: string): Promise<AiFilterResult> {
    ...
    return JSON.parse(textOutput.trim()) as AiFilterResult;
  }

  private async callOpenAI(queryText: string): Promise<AiFilterResult> {
    ...
    return JSON.parse(content.trim()) as AiFilterResult;
  }

  // Phương thức phân tích regex giả lập phòng hờ trường hợp không có API Key
  private parseMock(queryText: string): AiFilterResult {
    ...
    // Tìm thành phố
    if (text.includes('đà lạt') || text.includes('dalat')) result.city = 'Đà Lạt';
    else if (text.includes('đà nẵng') || text.includes('da nang')) result.city = 'Đà Nẵng';
    else if (text.includes('nha trang')) result.city = 'Nha Trang';
    else if (text.includes('phú quốc') || text.includes('phu quoc')) result.city = 'Phú Quốc';
    else if (text.includes('hồ chí minh') || text.includes('sài gòn') || text.includes('hcm')) result.city = 'Thành phố Hồ Chí Minh';
    else if (text.includes('hà nội') || text.includes('ha noi')) result.city = 'Thành phố Hà Nội';
    ...
    // Tìm tiện ích
    if (text.includes('hồ bơi') || text.includes('bể bơi')) result.amenities.push('Hồ bơi');
    if (text.includes('đậu xe') || text.includes('đỗ xe') || text.includes('ô tô')) result.amenities.push('Bãi đỗ xe');
    if (text.includes('wifi') || text.includes('mạng')) result.amenities.push('Wifi miễn phí');
    if (text.includes('gym') || text.includes('thể hình')) result.amenities.push('Phòng Gym / Thể hình');
    if (text.includes('điều hòa') || text.includes('máy lạnh')) result.amenities.push('Điều hòa nhiệt độ');
    if (text.includes('ăn sáng') || text.includes('nhà hàng')) result.amenities.push('Nhà hàng ăn uống');
    if (text.includes('spa') || text.includes('massage')) result.amenities.push('Dịch vụ Spa / Massage');
    if (text.includes('bar') || text.includes('rượu')) result.amenities.push('Quầy bar / Lounge');
    ...
    // Tìm địa danh
    if (text.includes('hồ xuân hương') || text.includes('xuan huong')) result.landmark = 'Hồ Xuân Hương';
    if (text.includes('sân bay')) result.landmark = 'Sân bay';
    if (text.includes('bãi biển') || text.includes('gần biển')) result.landmark = 'bãi biển';
