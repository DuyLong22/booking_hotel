...
class StatsController {
    async getAdminStats(req, res, next) {
        try {
            ...
            const totalHotels = await database_1.default.hotel.count();
            const totalRooms = await database_1.default.room.count();
            const totalOwners = await database_1.default.user.count({ where: { role: client_1.Role.HOTEL_OWNER } });
            const totalCustomers = await database_1.default.user.count({ where: { role: client_1.Role.CUSTOMER } });
            // Card 5-6: Bookings Counts
            const todayBookings = await database_1.default.booking.count({
                where: { createdAt: { gte: startOfToday } }
            });
            const monthlyBookings = await database_1.default.booking.count({
                where: { createdAt: { gte: startOfMonth } }
            });
            // Card 7-8: Revenue
            const todayBookingsList = await database_1.default.booking.findMany({
                where: {
                    createdAt: { gte: startOfToday },
                    status: { notIn: [client_1.BookingStatus.CANCELLED, client_1.BookingStatus.REFUNDED] }
                },
                select: { finalPrice: true }
            });
            const revenueToday = todayBookingsList.reduce((sum, b) => sum + Number(b.finalPrice), 0);
            const monthBookingsList = await database_1.default.booking.findMany({
                where: {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const dayBookings = await database_1.default.booking.findMany({
                    where: {
            ...
            const paymentDistribution = await database_1.default.payment.groupBy({
                by: ['method'],
                _count: { _all: true },
            ...
            if (pieData.length === 0) {
                pieData.push({ name: 'Credit Card', value: 50, color: '#0194f3' }, { name: 'VietQR', value: 30, color: '#10B981' }, { name: 'Khác', value: 20, color: '#F59E0B' });
            }
    ...
    async getOwnerStats(req, res, next) {
        try {
            ...
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            // Find owner's hotels
            const myHotels = await database_1.default.hotel.findMany({
                where: { ownerId },
                select: { id: true }
            });
            ...
            // Query real bookings
            const ownerBookings = await database_1.default.booking.findMany({
                where: {
                    bookingItems: {
                        some: {
                            roomType: {
            ...
            const upcomingCheckIn = ownerBookings.filter(b => b.status === client_1.BookingStatus.CONFIRMED && b.checkInDate >= startOfToday).length;
            // Stat 3: Upcoming check-out
            const upcomingCheckOut = ownerBookings.filter(b => b.status === client_1.BookingStatus.CHECKED_IN && b.checkOutDate >= startOfToday).length;
            // Rooms stats
            const totalRooms = await database_1.default.room.count({
                where: {
                    roomType: {
            ...
            const occupiedRooms = ownerBookings.filter(b => b.status === client_1.BookingStatus.CHECKED_IN).length;
            const availableRooms = Math.max(0, totalRooms - occupiedRooms);
            // Stat 6: Revenue today
            const revenueToday = ownerBookings
                .filter(b => b.createdAt >= startOfToday && b.status !== client_1.BookingStatus.CANCELLED)
                .reduce((sum, b) => sum + Number(b.finalPrice), 0);
            // Stat 7: Revenue month
            const revenueMonth = ownerBookings
                .filter(b => b.createdAt >= startOfMonth && b.status !== client_1.BookingStatus.CANCELLED)
                .reduce((sum, b) => sum + Number(b.finalPrice), 0);
            // Stat 8: Average Rating
            const reviews = await database_1.default.review.aggregate({
            ...
            const cancelledCount = ownerBookings.filter(b => b.status === client_1.BookingStatus.CANCELLED).length;
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const dayStart = new Date(d.getFullYear(), d.getMonth(), d.getDate());
                const dayEnd = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 23, 59, 59);
                const dayBookings = ownerBookings.filter(b => b.createdAt >= dayStart && b.createdAt <= dayEnd && b.status !== client_1.BookingStatus.CANCELLED);
                const dayRevenue = dayBookings.reduce((sum, b) => sum + Number(b.finalPrice), 0);
                const formattedDate = `${d.getDate().toString().padStart(2, '0')}/${(d.getMonth() + 1).toString().padStart(2, '0')}`;
                chartData.push({
                    name: formattedDate,
            ...
            const roomTypesData = await database_1.default.roomType.findMany({
                where: { hotelId: { in: myHotelIds } },
                include: { rooms: true }
            });
            ...
            for (let idx = 0; idx < roomTypesData.length; idx++) {
                ...
                const occupiedCount = await database_1.default.booking.count({
                    where: {
            ...
            if (occupancyData.length === 0) {
                occupancyData.push({ name: 'Chưa có hạng phòng', rate: 0, color: '#94A3B8' });
            }
