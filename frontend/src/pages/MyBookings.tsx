  ...
  const getStatusBadge = (status: string, booking?: Booking) => {
    ...
    switch (status) {
      ...
      case 'CONFIRMED':
        return (
          <div className="flex flex-col items-end gap-1">
            <span className="bg-green-50 text-green-700 px-2 py-0.5 rounded font-bold text-[10px] flex items-center gap-1">
              <CheckCircle2 className="w-3 h-3" /> Đã giữ phòng
            </span>
            {isPaid ? (
              <span className="bg-emerald-100 text-emerald-800 text-[9px] px-1.5 py-0.5 rounded font-bold">Đã TT Online</span>
            ) : isPayAtHotel ? (
              <span className="bg-amber-100 text-amber-800 border border-amber-200 text-[9px] px-1.5 py-0.5 rounded font-bold">
                🏨 Thanh toán tại khách sạn
              </span>
            ) : (
              <span className="bg-slate-100 text-slate-600 text-[9px] px-1.5 py-0.5 rounded font-bold">Chưa TT</span>
            )}
          </div>
        );
      case 'CHECKED_IN':
      ...
      case 'CANCELLED': {
        ...
        return (
          <div className="flex flex-col items-end gap-1">
            <span className="bg-red-50 text-red-700 px-2 py-0.5 rounded font-bold text-[10px] flex items-center gap-1">
              <AlertTriangle className="w-3 h-3" /> Đã hủy
            </span>
            {isRefunded && (
              <span className="bg-amber-100 text-amber-800 border border-amber-300 text-[9px] px-1.5 py-0.5 rounded font-bold flex items-center gap-1 shadow-xs">
                💸 Đã hoàn tiền (VNPay)
              </span>
            )}
          </div>
                ...
                <div className="flex justify-between items-center pt-3 border-t border-slate-50">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {booking.status === 'CONFIRMED' && (
                      <>
                        ...
                        {booking.payment?.status !== 'COMPLETED' && (
                          <button
                            onClick={() => handlePayNow(booking)}
                            className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-[10px] px-3 py-1.5 rounded-premium transition-colors shadow flex items-center gap-1"
                          >
                            <CreditCard className="w-3.5 h-3.5" /> Thanh toán Online ngay
                          </button>
                        )}
                      </>
                    )}
                    ...
                    {(booking.status === 'PENDING' || booking.status === 'PAYMENT_PROCESSING' || booking.status === 'CONFIRMED') && (
                      <button
                        onClick={async () => {
                          if (window.confirm('Bạn có chắc chắn muốn hủy đơn đặt phòng này? Đặt phòng của bạn sẽ bị hủy và phòng sẽ được giải phóng cho khách hàng khác.')) {
                            try {
